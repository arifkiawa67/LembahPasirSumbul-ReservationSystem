...
$query = "SELECT tbro.id_offline_reservation, tbr.name_off_tourist, tbr.phoneno_off_tourist, tbs.name_services, tbs.desc_services, tbs.price_services, tbro.visit_start_date, tbro.visit_end_date, tbst.title_status, tbst.desc_status, tbro.id_status FROM tb_offline_reservaion tbro JOIN tb_offline_tourist tbr ON tbr.id_off_tourist = tbro.id_offline_tourist JOIN tb_services tbs ON tbs.id_services = tbro.id_services JOIN tb_status tbst ON tbst.id_status = tbro.id_status 
          WHERE tbro.id_offline_reservation = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($new_status == 5) {
        ...
        $updateQuery = "UPDATE tb_offline_reservaion SET id_status = ?, visit_end_date = ? WHERE id_offline_reservation  = ?";
        $updateStmt = $conn->prepare($updateQuery);
        $updateStmt->bind_param("isi", $new_status, $current_date, $id);
    } else {
        // For other status updates
        $updateQuery = "UPDATE tb_offline_reservaion SET id_status = ? WHERE id_offline_reservation  = ?";
        $updateStmt = $conn->prepare($updateQuery);
...
<body>
    <div class="container">
        ...
        <table>
            <tr>
                <th>ID Reservasi</th>
                <td><?= htmlspecialchars($data['id_offline_reservation']) ?></td>
            </tr>
            <tr>
                <th>Nama</th>
                <td><?= htmlspecialchars($data['name_off_tourist']) ?></td>
            </tr>
            <tr>
                <th>Nomor Telepon</th>
                <td><?= htmlspecialchars($data['phoneno_off_tourist']) ?></td>
            </tr>
            <tr>
                <th>Layanan</th>
